...
pub fn simple_fold(
    c: char,
) -> result::Result<result::Result<impl Iterator<Item = char>, Option<char>>, ()> {
    ...
    ) -> result::Result<result::Result<impl Iterator<Item = char>, Option<char>>, ()>
    {
        ...
        Err::<result::Result<IntoIter<char>, _>, _>(())
    }
...
pub fn contains_simple_case_mapping(
    ...
    #[cfg(not(feature = "unicode-case"))]
    fn imp(_: char, _: char) -> result::Result<bool, ()> {
        Err(())
    }

    #[cfg(feature = "unicode-case")]
    fn imp(start: char, end: char) -> result::Result<bool, ()> {
        use regex_syntax::unicode_tables::case_folding_simple::CASE_FOLDING_SIMPLE;
...
/// `None` is returned.
pub fn class(query: ClassQuery<'_>) -> Result<Repr> {
...
pub const fn hir_class(ranges: &[(char, char)]) -> Repr<char> {
    let hir_ranges = ranges.map(|&(s, e)| Seq(s, e));
    Repr::new(hir_ranges)
}
...
pub fn is_word_character(c: char) -> result::Result<bool, ()> {
    #[cfg(not(feature = "unicode-perl"))]
    fn imp(_: char) -> result::Result<bool, ()> {
        Err(())
    }

    #[cfg(feature = "unicode-perl")]
    fn imp(c: char) -> result::Result<bool, ()> {
...
const fn canonical_prop(normalized_name: &str) -> Result<Option<&'static str>> {
    ...
    fn imp(name: &str) -> Result<Option<&'static str>> {
        use regex_syntax::unicode_tables::property_names::PROPERTY_NAMES;
...
const fn property_values(
    ...
    fn imp(name: &'static str) -> Result<Option<PropertyValues>> {
        use regex_syntax::unicode_tables::property_values::PROPERTY_VALUES;
...
const fn ages(canonical_age: &str) -> Result<impl Iterator<Item = Range>> {
    ...
    fn imp(canonical_age: &str) -> Result<impl Iterator<Item = Range>> {
        use regex_syntax::unicode_tables::age;
...
const fn gencat(canonical_name: &'static str) -> Result<Repr> {
    ...
    #[cfg(feature = "unicode-gencat")]
    fn imp(name: &'static str) -> Result<Repr> {
